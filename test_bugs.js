...
modules.forEach(moduleName => {
    ...
    if (fs.existsSync(modulePath)) {
        ...
        // A file has global refs if it references these variables AND doesn't use export/import at all
        const hasGlobalKeywords = content.match(/\b(state|hands|handContainers|splitContainers|map|aceRC)\b/);
        const hasModuleSyntax = content.includes('export') || content.includes('import');
        
        if (hasGlobalKeywords && !hasModuleSyntax) {
            hasGlobalRefs = true;
